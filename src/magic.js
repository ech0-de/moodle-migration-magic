...
export async function processBackup(file, patchData = false, log = console.log) {
  ...
  if (!patchData) {
      ...
      // const packer = new tar.Pack({ gzip: true, onwarn: logger });
      // const parser = new tar.Parse({ strict: true, onwarn: logger });
      // const gzip = zlib.createGzip();

      for (const entry of items) {
        if (entry.name.endsWith('/')) {
          writer.addFolder(entry.name);
        } else if (patchedFiles.has(entry.name)) {
          const content = serializer.serializeToString(patchedFiles.get(entry.name));
          writer.addFile(entry.name, content, { mtime });
        } else {
          writer.addFile(entry.name, reader.getFileBlob(entry.name), { mtime });
        }
      }

      const blob = await writer.write();
      const compressedStream = blob.stream().pipeThrough(new CompressionStream('gzip'));
      const compressedBackup = await new Response(compressedStream).blob();
